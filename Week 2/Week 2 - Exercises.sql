...
SELECT orderid, orderdate, custid, empid FROM SALES.ORDERS WHERE MONTH(ORDERDATE) = 06;
...
SELECT orderid, EOMONTH(orderdate), custid, empid FROM SALES.ORDERS;
...
SELECT empid, lastname FROM HR.EMPLOYEES WHERE lastname LIKE '[a, z]%';
...
--and then the data before the order date 2016-05-01 will be sorted.
...
SELECT empid, firstname, lastname, titleofcourtesy, gender = CASE
...
SELECT custid, region FROM Sales.Customers
    ORDER BY IIF(region IS NULL, 1, 0), region;
...
DECLARE @cnt INT = ( SELECT TOP 1 COUNT(orderid) AS C  FROM SALES.ORDERS GROUP BY custid
ORDER BY C DESC );
WITH TMP AS (
    SELECT C.custid, orderid, orderdate, empid, COUNT(orderid) OVER ( PARTITION BY C.custid ORDER BY C.custid ) AS [count]
    FROM Sales.Orders O
        JOIN Sales.Customers C on C.custid = O.custid
        GROUP BY C.custid, orderid, orderdate, empid
)
...
SELECT empid, firstname, lastname FROM HR.Employees
    WHERE empid NOT IN ( SELECT empid FROM Sales.Orders WHERE orderdate >= '2016.05.1' );
...
WITH LDA AS (
    SELECT custid, MAX(orderdate) AS orderdate FROM Sales.Orders
    GROUP BY custid
)
SELECT L.custid, O.orderid, L.orderdate, O.empid
    FROM LDA L
    JOIN Sales.Orders O ON L.custid = O.custid AND L.orderdate = O.orderdate
    ORDER BY L.custid
...
SELECT DISTINCT O.custid, companyname FROM Sales.Orders O
JOIN Sales.Customers C ON C.custid = O.custid
WHERE YEAR(orderdate) = 2015 AND O.custid NOT IN (
    SELECT custid FROM Sales.Orders WHERE YEAR(orderdate) = 2016
    );
...
WITH ORD AS (
    SELECT O.orderid, O.custid, OD.productid FROM Sales.Orders O
INNER JOIN Sales.OrderDetails OD ON O.orderid = OD.orderid
WHERE productid = 12
)
SELECT DISTINCT O.custid, C.companyname FROM ORD O
...
SELECT *, SUM(qty) OVER ( PARTITION BY custid ORDER BY ordermonth ) AS runqty
...
WITH TMP AS
(
	SELECT CUSTID, ORDERDATE, ORDERID, LAG(ORDERDATE) OVER(PARTITION BY CUSTID ORDER BY CUSTID, ORDERDATE) AS PREVORDERDATE  FROM SALES.ORDERS
)
	SELECT CUSTID, ORDERDATE, ORDERID, DATEDIFF(DAY, PREVORDERDATE, ORDERDATE) AS DIFF FROM TMP;
...
WITH TMP AS (
    SELECT Employee_ID, COUNT(Project_ID) AS [COUNT] FROM dbo.Projects
GROUP BY Employee_ID
)
SELECT TOP 3 ID, [NAME], SUM(Salary) AS Salary FROM dbo.Employee E
INNER JOIN TMP T ON T.Employee_ID = ID
WHERE [COUNT] >= 3
GROUP BY ID, [NAME]
ORDER BY [Salary];
...
-- This is a real Google Interview question, Level = Easy

WITH TMP AS (
    SELECT DISTINCT Parent, COUNT(DISTINCT ChildGender) AS [COUNT] FROM dbo.tblPerson
GROUP BY Parent
)
SELECT Parent FROM TMP WHERE [COUNT] = 2;
